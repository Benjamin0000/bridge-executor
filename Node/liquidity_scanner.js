...
// // -------------------- CONFIG --------------------
// const BACKEND_URL = process.env.BACKEND_URL || "https://hedera-api.kivon.io/api/add-liquidity";
...
const ALCHEMY_WSS = `wss://eth-mainnet.g.alchemy.com/v2/${process.env.NEXT_PUBLIC_ALCHEMY_API_KEY}`;
const WATCH_ADDRESS = "0xYourAddress".toLowerCase();
const SERVER_URL = "https://yourserver.com/api/deposit";

const provider = new WebSocketProvider(ALCHEMY_WSS);

console.log(`📡 Monitoring all ETH deposits to ${WATCH_ADDRESS}...`);

let processed = new Set(); // Prevent double sending

provider.on("block", async (blockNumber) => {
  console.log("🔍 Checking block:", blockNumber);

  const block = await provider.getBlock(blockNumber, true); // true = include full txs
  if (!block || !block.transactions) return;

  for (const tx of block.transactions) {
    if (tx.to && tx.to.toLowerCase() === WATCH_ADDRESS) {
      const hash = tx.hash;
      ...
      try {
        ...
        console.error("❌ Failed to send to server:", err.message);
      }
    }
  }
});
